...
export default function SchoolStreamAssessmentPage() {
  ...
  const [savedHistory, setSavedHistory] = useState<StreamAssessmentResponse[]>([]);
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("devgya_stream_assessments");
      if (stored) {
  ...
  const saveToLocalHistory = (paper: StreamAssessmentResponse) => {
    try {
      const existing = savedHistory.filter(p => p.id !== paper.id);
      const updated = [paper, ...existing].slice(0, 10);
      setSavedHistory(updated);
      localStorage.setItem("devgya_stream_assessments", JSON.stringify(updated));
    } catch {
      // ignore
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    const stageInterval = setInterval(() => {
      setGenerationStage(prev => (prev < 4 ? prev + 1 : prev));
    }, 1800);
  ...
  const handleDownloadPDF = async (includeAnswers: boolean) => {
    ...
    try {
      await downloadStreamAssessmentPDF(assessmentPaper, includeAnswers);
    } catch (e: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
        ...
        {assessmentPaper && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3.5 py-2 text-xs font-bold text-slate-600 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl transition-all shadow-2xs flex items-center gap-1.5"
            >
      ...
      {/* 2. MAIN LAYOUT: FORM / GENERATION OR PREVIEW */}
      {!assessmentPaper ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xs p-6 space-y-6">
              ...
              <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Configure grade cohort, question weights, and school branding.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700">Class / Cohort Level</label>
                  <select
                    value={className}
                  ...
                  >
                    <option value="Class 11">Class 11 (Stream Selection & Aptitude)</option>
                    <option value="Class 12">Class 12 (Board & Career Stream Diagnostic)</option>
                    <option value="Class 10 -> 11 Transition">Class 10 → 11 Transition (Stream Aptitude)</option>
                  </select>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="e.g. Senior Secondary Stream Aptitude Assessment"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                  />
                </div>
                ...
                {/* Duration */}
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700">Time Allowed (Minutes)</label>
                  <select
              ...
              {/* 3. QUESTION MIX & STRUCTURE MATRIX */}
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-extrabold text-slate-800">
                    Question Distribution Across Streams
                  </label>
                  <span className="text-[11px] font-bold text-indigo-600">
                    Balanced across Science • Commerce • Humanities
                  </span>
                </div>
          ...
          {/* RIGHT 1 COLUMN: LIVE SUMMARY & PREVIOUS PAPERS */}
            ...
            {savedHistory.length > 0 && (
              <div className="bg-white rounded-3xl border border-slate-200/90 p-5 space-y-3 shadow-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-bold text-slate-400">{savedHistory.length} saved</span>
                </div>
                ...
                <div className="space-y-2">
                  {savedHistory.slice(0, 3).map((item, idx) => (
                    <div 
                      key={item.id || idx}
                      onClick={() => setAssessmentPaper(item)}
                      className="p-3 rounded-2xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all cursor-pointer group"
                    >
          ...
          <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-3xl p-5 text-white flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-lg">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-300">
                {assessmentPaper.class_name} &bull; {assessmentPaper.total_marks} Marks &bull; {assessmentPaper.time_allowed_mins} Mins &bull; {assessmentPaper.questions.length} Questions
              </p>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xs p-6 sm:p-8 space-y-6">
              ...
              <div className="text-center space-y-1.5 border-b border-slate-200 pb-5">
                ...
                <div className="flex items-center justify-center gap-4 text-xs font-bold text-slate-600 pt-1">
                  <span>Class: {assessmentPaper.class_name}</span>
                  <span>&bull;</span>
                  <span>Max Marks: {assessmentPaper.total_marks}</span>
                  <span>&bull;</span>
                  <span>Time Allowed: {assessmentPaper.time_allowed_mins} Mins</span>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs text-slate-700 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>Candidate Name: _______________________________</div>
                <div>Roll / Scholar No: _________________________</div>
                <div>Date: _________________________________________</div>
                <div>Target Stream: [&nbsp;] Science &nbsp; [&nbsp;] Commerce &nbsp; [&nbsp;] Humanities &nbsp; [&nbsp;] Undecided</div>
              </div>

              {/* Instructions */}
              <div className="space-y-1.5 text-xs text-slate-600">
                <div className="font-bold text-slate-800">General Instructions:</div>
                {assessmentPaper.instructions.map((inst, idx) => (
              ...
              <div className="space-y-6">
                {assessmentPaper.questions.map((q) => {
                  ...
                  const isHumanities = q.stream === "humanities";
