...
export function MobileStudentDashboard() {
  ...
  const studentTools = [
    { name: "English Speaking Coach", sub: "Live Spoken English Fluency", href: "/dashboard/english-coach", icon: Headphones, color: "text-cyan-600", bg: "bg-cyan-50", border: "border-cyan-100", type: "English Coach" },
      ...
      <div className="bg-gradient-to-br from-[#1b1c54] via-[#2a1b6d] to-[#12163b] text-white p-5 rounded-[28px] shadow-xl border border-indigo-700/40 relative overflow-hidden space-y-3">
        ...
        <div className="flex items-center gap-2 pt-1 relative z-10">
          ...
          <Link
            ...
            className="p-2.5 px-4 bg-white/15 hover:bg-white/25 text-white rounded-2xl border border-white/20 transition-all flex items-center justify-center active:scale-95 cursor-pointer text-xs font-bold gap-1.5"
              ...
              {allowedStudentTools.map((tool, idx) => {
                ...
                return (
                  <Link
                    key={idx}
                    href={tool.href}
                    className="p-3 bg-white rounded-2xl border border-slate-100 shadow-xs hover:shadow-md transition-all flex flex-col justify-between space-y-2 active:scale-95 cursor-pointer relative"
                  >
