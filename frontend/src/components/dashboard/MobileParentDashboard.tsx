...
export function MobileParentDashboard() {
  ...
  const parentTools = [
    { name: "AI Parenting Coach", sub: "Behavior, Motivation & Routines", href: "/dashboard/agents?agent=parent_coach", icon: HeartHandshake, color: "text-rose-600", bg: "bg-rose-50", border: "border-rose-100", category: "coach", badge: "Super Agent" },
    { name: "Subject Marks Radar", sub: "Weak Spot & Progress Tracker", href: "/dashboard/agents?agent=analytics_assistant", icon: BarChart3, color: "text-indigo-600", bg: "bg-indigo-50", border: "border-indigo-100", category: "radar", badge: "Analytics" },
    { name: "Video Consultation", sub: "1-on-1 Call with Teacher Mentor", href: "/dashboard/video-consultation", icon: Video, color: "text-purple-600", bg: "bg-purple-50", border: "border-purple-100", category: "call", badge: "Live Mentor" },
    { name: "English Coach", sub: "Communication & Public Speaking", href: "/dashboard/agents?agent=english_coach", icon: MessageSquare, color: "text-violet-600", bg: "bg-violet-50", border: "border-violet-100", category: "coach", badge: "Language" },
    { name: "Career Pathways AI", sub: "Streams, Exams & Degree Guide", href: "/dashboard/agents?agent=career_counselor", icon: Compass, color: "text-cyan-600", bg: "bg-cyan-50", border: "border-cyan-100", category: "guide", badge: "Future Prep" },
    { name: "Safe Learning Shield", sub: "Ad-Free & Screen-Time Filtered", href: "/safety-standards", icon: ShieldCheck, color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-100", category: "guide", badge: "Protected" },
  ];
  ...
  const filteredTools = useMemo(() => {
    if (activeCategory === "all") return parentTools;
    return parentTools.filter(t => t.category === activeCategory);
  }, [activeCategory]);
    ...
    <div className="space-y-4 pb-24 animate-in fade-in duration-300 md:hidden px-1">
      ...
      <div className="bg-gradient-to-br from-[#2a133d] via-[#3d1a52] to-[#12163b] text-white p-5 rounded-[28px] shadow-xl border border-pink-700/40 relative overflow-hidden space-y-3">
        ...
        <div className="flex items-center justify-between relative z-10">
          <span className="text-[10px] font-black uppercase tracking-wider bg-white/15 px-3 py-1 rounded-full text-rose-100 border border-white/10 backdrop-blur-md">
            PARENT OS HUB
          </span>
        ...
        <div className="flex items-center justify-between gap-2 relative z-10 pt-1">
          <div className="space-y-1 max-w-[62%]">
            <h1 className="text-xl font-black tracking-tight leading-tight">
              Welcome, <br />
              <span className="text-amber-300">{user?.name || "Parent"}! 🤝</span>
            </h1>
          ...
          <div className="relative shrink-0 w-24 h-24 flex items-center justify-center">
            <div className="w-20 h-20 bg-gradient-to-tr from-rose-500 via-pink-600 to-indigo-600 rounded-3xl rotate-6 shadow-lg flex flex-col items-center justify-center p-2 border border-white/20">
              <HeartHandshake className="w-8 h-8 text-amber-300 drop-shadow animate-pulse" />
        ...
        <div className="flex items-center gap-2.5 pt-1 relative z-10">
          <Link
            ...
            className="flex-1 py-2.5 bg-white text-rose-950 font-black text-xs rounded-2xl shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all hover:bg-slate-50 cursor-pointer"
          >
          ...
          <Link
            ...
            title="Book Video Consultation Call"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs px-4 py-3 flex items-center justify-between gap-2.5 transition-all focus-within:border-rose-500 focus-within:ring-2 focus-within:ring-rose-100">
        <div className="flex items-center gap-2.5 flex-1 text-slate-400 text-xs">
          ...
          <input
            ...
            placeholder="Search parenting advice, marks radar, routine tips..."
      ...
      {hasSearch ? (
        <div className="space-y-3 bg-white rounded-3xl p-4 border border-slate-200 shadow-sm">
          ...
          {matchingTools.length === 0 ? (
            <div className="py-8 text-center space-y-1">
              ...
              <p className="text-[11px] text-slate-400">Try searching "Coach", "Marks", "Routine" or "Call"</p>
            </div>
          ) : (
            <div className="space-y-2">
              {matchingTools.map((t, idx) => (
                <Link
                  key={idx}
                  href={t.href}
                  className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 hover:bg-rose-50/50 border border-slate-200/80 transition active:scale-98"
                >
        ...
        <>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar text-xs font-bold px-0.5">
            {[
              { id: "all", label: "All Hub (6)" },
              { id: "coach", label: "🤝 AI Coach (2)" },
              { id: "radar", label: "📊 Marks Radar (1)" },
              { id: "call", label: "📹 Mentor Calls (1)" },
              { id: "guide", label: "🛡️ Safety & Guides (2)" },
            ].map((cat) => (
              <button
                key={cat.id}
                type="button"
          ...
          {/* 5. PRIMARY PARENTING TOOLS GRID */}
          <div className="grid grid-cols-2 gap-2.5">
            {filteredTools.map((t, idx) => (
              ...
              >
                ...
                <div>
                  <h3 className="text-xs font-black text-slate-900 group-hover:text-rose-600 transition truncate">
                    {t.name}
                  </h3>
