...
async def generate_stream_assessment_pdf_endpoint(payload: Dict[str, Any] = Body(...), include_answers: bool = False):
    ...
    try:
        ...
        curr_name = (payload.get("school_name") or "").strip()
        is_generic_name = not curr_name or curr_name in ("Apex International School", "DEVGYA GLOBAL ACADEMY", "School")

        # Auto-resolve school_name & school_logo if missing or generic
        if (is_generic_name or not payload.get("school_logo")) and user_email:
            try:
                ...
                if sch:
                    if is_generic_name and sch.get("school_name"):
                        payload["school_name"] = sch["school_name"]
                        curr_name = sch["school_name"]
                    if not payload.get("school_logo") and sch.get("logo_url"):
                        payload["school_logo"] = sch.get("logo_url")
            except Exception:
                pass

        if not payload.get("school_logo") and curr_name and not is_generic_name:
            try:
                from services.recruitment_service import recruitment_service
                target_name = curr_name.lower()
